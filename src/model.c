...
void LoadModel(Model *model, const char *file) {
	...
	glm_vec3_copy((vec3){0.0, 0.0, 0.0}, model->position);
	glm_vec3_copy((vec3){0.0, 0.0, 0.0}, model->rotation);
	glm_vec3_copy((vec3){1.0, 1.0, 1.0}, model->scale);
	model->rotationAngle = 0.0;
